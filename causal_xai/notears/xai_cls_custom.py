...
def cal_cross_entropy_loss(output, target, cross_entropy_loss):
    n = target.shape[0]
    loss = cross_entropy_loss(output, target)/n
    return loss
...
def dual_ascent_step(model, X, wandb, lambda_cls, lambda1, lambda2, rho, alpha, h, rho_max):
    ...
    while rho < rho_max:
        def closure():
            ...
            loss = cal_cross_entropy_loss(X_hat, X_torch, cross_entropy_loss)
            h_val = model.h_func()
