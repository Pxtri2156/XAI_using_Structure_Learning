...
class NotearsMLPRegression(nn.Module):
    def __init__(self, dims, bias=True):
        super(NotearsMLPRegression, self).__init__()
        assert len(dims) >= 2
        assert dims[-1] == 1
        d = dims[0]
        self.dims = dims
...
def main(args):
    ...
    for seed in range(args.no_seeds):
        ...
        # Modeling
        model = NotearsMLPRegression(dims=[d, 10, 1], bias=True)
        W_est = notears_nonlinear(model, X_train, wandb, args.lambda_reg, args.lambda1, args.lambda2)
                # Saving model 
        model_path = os.path.join(out_folder, "reg_model.pickle")
        ...
        reg_predict, target = pred_reg(X_test, model)
    ...
    r2_score_lst = np.array(r2_score_lst)
    final_reg_results = { 'test_loss': {'mean': np.mean(test_loss_lst), 
                                        'std': np.std(test_loss_lst)},
                         'r2_scores': {'mean': np.mean(r2_score_lst),
                                       'std':np.std(r2_score_lst)}}
